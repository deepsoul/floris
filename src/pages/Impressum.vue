<template>
  <v-container class="my-2 my-md-12">
    <h2 class="font-weight-bold mt-0 mb-md-6">Impressum</h2>
    <v-card class="pa-4 mb-4" elevation="1">
      <v-card-text>
        <strong>Enddarmzentrum Bietigheim</strong><br />
        Dr. med. Oliver Schunter<br />
        Dr. med. Markus Piro<br />
        Floris Engelsman<br />
        <br />
        Bahnhofplatz 4<br />
        74321 Bietigheim-Bissingen<br />
        <br />
        Tel. 07142/53232<br />
        Fax 07142/54655<br />
        info@enddarmzentrum-bietigheim.de<br />
        <br />
        <strong>Berufsbezeichnung:</strong><br />
        <em>Dr. med. Oliver Schunter</em><br />
        Arzt, Arzt für Chirurgie-Viszeralchirugie-Proktologie (Verliehen in der
        Bundesrepublik Deutschland)<br />
        <br />
        <em>Dr. med. Markus Piro</em><br />
        Arzt, Arzt für Chirurgie-Viszeralchirurgie-Proktologie (Verliehen in der
        Bundesrepublik Deutschland)<br />
        <br />
        <em>Floris Engelsman</em><br />
        Arzt, Arzt für Viszeralchirurgie-Proktologie (Verliehen in der
        Bundesrepublik Deutschland)<br />
        <br />
        <strong>Zuständige Aufsichtsbehörde:</strong><br />
        Kassenärztliche Vereinigung Baden-Württemberg<br />
        Bezirksdirektion Stuttgart<br />
        Albstadtweg 11<br />
        70567 Stuttgart<br />
        <a href="https://www.kvbawue.de" target="_blank">www.kvbawue.de</a
        ><br />
        <br />
        <strong>Zuständige Kammer:</strong><br />
        Landesärztekammer Baden-Württemberg<br />
        Bezirksärztekammer Nordwürttemberg<br />
        Jahnstr. 5<br />
        70597 Stuttgart<br />
        <a href="https://www.aerztekammer-bw.de" target="_blank"
          >www.aerztekammer-bw.de</a
        ><br />
        <br />
        <strong>Berufsrechtliche Regelungen:</strong><br />
        Die Berufsordnung können Sie auf der Internetseite der Ärztekammer
        Baden-Württemberg unter
        <a href="https://www.aerztekammer-bw.de" target="_blank"
          >www.aerztekammer-bw.de</a
        >
        einsehen.<br />
        <br />
        <strong>Inhaltlich Verantwortlicher gemäß § 6 MDStV:</strong><br />
        Dr. Markus Piro<br />
        <br />
        <strong>Haftungshinweis:</strong><br />
        Trotz sorgfältiger inhaltlicher Kontrolle übernehmen wir keine Haftung
        für die Inhalte externer Links. Für den Inhalt der verlinkten Seiten
        sind ausschließlich deren Betreiber verantwortlich.
      </v-card-text>
    </v-card>
  </v-container>
</template>
<style lang="scss" scoped>
strong {
  font-family: 'Montserrat', sans-serif !important;
  font-size: 1.1rem !important;
}
</style>
